...
class Sales extends MY_Controller
{
    ...
    public function repair_list($warehouse_id = null)
    {
        ...
        if($clear_filter == 'yes') {
        	$this->session->set_userdata('dt_search', '');
        	admin_redirect('sales/repair_list');
        }
        ...
        $this->data['dt_search'] = $this->session->userdata('dt_search');
    ...
    {
    	$this->session->set_userdata('dt_search', $this->input->post('sSearch', ''));
        $this->sma->checkPermissions('index');
...
//        $repair_link = anchor('admin/sales/repair_history/$1', '<i class="fa fa-history"></i> ' . lang('Repair_History'));
        $detail_link2 = anchor('admin/sales/modal_view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('Print_Sticker'), 'data-toggle="modal" data-target="#myModal"');
    ...
    public function repair_status($id)
    {
        ...
        if ($this->form_validation->run() == true) {
            $status = $this->input->post('status');
            $signature = $this->input->post('signature_name');
...
//        $repair_link = anchor('admin/sales/repair_history/$1', '<i class="fa fa-history"></i> ' . lang('Repair_History'));
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, ".$this->db->dbprefix('companies') .".name, phone, note, (grand_total+COALESCE(rounding, 0)), paid, (grand_total-paid) as balance,type_detail,sale_status, payment_status, companies.email as cemail")
                ->from('sales')
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, ".$this->db->dbprefix('companies') .".name, phone, note, (grand_total+COALESCE(rounding, 0)), paid, (grand_total+rounding-paid) as balance, type_detail,sale_status, payment_status, companies.email as cemail")
                ->from('sales')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->where('type_detail', 'Ready')
                ->where('DATEDIFF(now(),date) >=', 30)
